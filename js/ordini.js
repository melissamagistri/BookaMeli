$(document).ready(function () {
  ...
  $(".invia").click(function () {
    if (
      $(".selected + div > div > #titolo").val() != "" &&
      $(".selected + div > div > #recensione").val() != "" &&
      $('.selected + div > div > input[name="rate"]:checked').val()
    ) {
      var recensione = {
        nomeprodotto: $(".selected")
          .siblings(".image")
          .find(".imgordini")
          .attr("alt"),
        ...
        function (data, status) {
          if (status == "success") {
            ...
            $(".selected")
              .next()
              .html(
                "<p>Si è verificato un problema con il server, la preghiamo di riprovare piu tardi.</p>"
              );
          }
        }
      );
      ...
      alert(
        "Per aggiungere o modificare una recensione devi compilare tutti i campi!"
      );
    }
  });
    ...
    $.post(
      ...
      function (data, status) {
        if (status == "success") {
          ...
          $(".selected")
            .next()
            .html("<p>La recensione è stata eliminata con successo!</p>");
        } else {
          $(".selected")
            .next()
            .html(
              "<p>Si è stato un problema con il server, la preghiamo di riprovare piu tardi.</p>"
            );
        }
      }
    );
